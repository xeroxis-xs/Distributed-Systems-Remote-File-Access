...
public class Marshaller {
    ...
    // Marshalling: Convert fields into a byte array representation
    public static byte[] marshal(String message) {

        // Convert string to byte array using UTF-8 encoding
        byte[] messageBytes = message.getBytes(StandardCharsets.UTF_8);

        // Create byte array
        byte[] marshalledData = new byte[messageBytes.length];

        // Concatenate byte arrays
        System.arraycopy(messageBytes, 0, marshalledData, 0, messageBytes.length);
